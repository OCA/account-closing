...
class AccountAccount(osv.osv):
    ...
    _columns = {'currency_reevaluation': fields.boolean("Allow Currency Reevaluation")}

    _defaults = {'currency_reevaluation': False}
    ...
    def compute_balances(self, cursor, uid, ids, context, query):
        ...
        #                                      context=context,
        #                                      query=query)
        return self.__compute(cursor, uid, ids,
                              ['balance', 'foreign_balance'],
                              context=context,
                              query=query)

    #Copy of account.account __compute it needs to be slightly changed
    def __compute(self, cr, uid, ids, field_names, arg=None, context=None,
        ...
        # I change this previous convention to be able to compute selected movelines by currency in case currency_id appears in filters
        if query and "l.currency_id" in query:
            mapping.update({'foreign_balance': "COALESCE(SUM(l.amount_currency), 0) as foreign_balance"})
