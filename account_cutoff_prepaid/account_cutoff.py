...
class account_cutoff(osv.Model):
    ...
    _columns = {
        'source_journal_ids': fields.many2many('account.journal',
            id1='cutoff_id', id2='journal_id',
    ...
    def _get_default_source_journals(self, cr, uid, context=None):
        ...
        if type in mapping.keys():
            src_journal_ids = journal_obj.search(cr, uid, [('type', 'in', mapping[type])])
            if src_journal_ids:
                res = src_journal_ids
        return res
    ...
    _sql_constraints = [
        ('date_type_company_uniq', 'unique(cutoff_date, company_id, type)', 'A cut-off of the same type already exists with this cut-off date !'),
    ]
    ...
    def get_prepaid_lines(self, cr, uid, ids, context=None):
        ...
        cur_cutoff = self.read(cr, uid, ids[0], ['line_ids', 'source_journal_ids', 'cutoff_date', 'company_id', 'type', 'company_currency_id'], context=context)
        src_journal_ids = cur_cutoff['source_journal_ids']
        if not src_journal_ids:
            raise orm.except_orm(_('Error :'), _("You should set at least one Source Journal."))
        ...
        # Create mapping dict
        mapping = mapping_obj._get_mapping_dict(cr, uid, cur_cutoff['company_id'][0], cur_cutoff['type'], context=context)

        # Loop on selected account move lines to create the cutoff lines
        for aml in aml_obj.read(cr, uid, aml_ids, ['credit', 'debit', 'start_date', 'end_date', 'account_id', 'analytic_account_id', 'partner_id', 'name'], context=context):
            ...
            if aml['start_date'] > cutoff_date_str:
                ...
                if total_days:
                    cutoff_amount = -1 * (aml['credit'] - aml['debit']) * after_cutoff_days / total_days
                else:
                    raise orm.except_orm(_('Error :'), "This should never happen. Total days should always be > 0")

            # we use account mapping here
            if aml['account_id'][0] in mapping.keys():
            ...
            line_obj.create(cr, uid, {
                ...
                'analytic_account_id': aml['analytic_account_id'] and aml['analytic_account_id'][0] or False,
    ...
    def _inherit_default_cutoff_account_id(self, cr, uid, context=None):
        if context is None:
            context = {}
        account_id = super(account_cutoff, self)._inherit_default_cutoff_account_id(cr, uid, context=context)
        type = context.get('type')
...
class account_cutoff_line(osv.Model):
    ...
    _columns = {
        'move_line_id': fields.many2one('account.move.line', 'Accout Move Line',
            readonly=True),
        'move_date': fields.related('move_line_id', 'date', type='date',
        ...
        'after_cutoff_days': fields.integer('Number of Days after Cut-off Date',
            readonly=True),
        }
