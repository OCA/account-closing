...
class account_invoice_line(orm.Model):
    ...
    def _check_start_end_dates(self, cr, uid, ids):
        for invline in self.browse(cr, uid, ids):
            if invline.start_date and not invline.end_date:
                raise orm.except_orm(_('Error :'), _("Missing End Date for invoice line with Description '%s'.") %(invline.name))
            if invline.end_date and not invline.start_date:
                raise orm.except_orm(_('Error :'), _("Missing Start Date for invoice line with Description '%s'.") %(invline.name))
            if invline.end_date and invline.start_date and invline.start_date > invline.end_date:
                raise orm.except_orm(_('Error :'), _("Start Date should be before or be the same as End Date for invoice line with Description '%s'.") %(invline.name))
    ...
    _constraints = [
        (_check_start_end_dates, "Error msg in raise", ['start_date', 'end_date', 'product_id']),
    ]
    ...
    def _check_start_end_dates(self, cr, uid, ids):
        for moveline in self.browse(cr, uid, ids):
            if moveline.start_date and not moveline.end_date:
                raise orm.except_orm(_('Error :'), _("Missing End Date for move line with Name '%s'.") %(moveline.name))
            if moveline.end_date and not moveline.start_date:
                raise orm.except_orm(_('Error :'), _("Missing Start Date for move line with Name '%s'.") %(moveline.name))
            if moveline.end_date and moveline.start_date and moveline.start_date > moveline.end_date:
                raise orm.except_orm(_('Error :'), _("Start Date should be before End Date for move line with Name '%s'.") %(moveline.name))
...
class account_invoice(orm.Model):
    ...
    def inv_line_characteristic_hashcode(self, invoice, invoice_line):
        ...
        return '%s-%s-%s' % (code, invoice_line.get('start_date', 'False'),
            ...
            for invline in invoice.invoice_line:
                if invline.product_id and invline.product_id.must_have_dates:
                    if not invline.start_date or not invline.end_date:
                        raise orm.except_orm(_('Error :'), _("Missing Start Date and End Date for invoice line with Product '%s' which has the property 'Must Have Start and End Dates'.") %(invline.product_id.name))
